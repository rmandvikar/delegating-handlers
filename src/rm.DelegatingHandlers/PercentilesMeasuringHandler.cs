...
namespace rm.DelegatingHandlers
{
	...
	public class PercentilesMeasuringHandler : DelegatingHandler
	{
		...
		internal int CalculateNextIntervalDelay(int intervalInSeconds)
		{
			...
			//// in some cases, the timer callback could fire multiple times in the same interval, so handle that
			//// if next timer callback fire is within 10%, just take the interval value
			//var adjust = false;
			//lock (locker)
			//{
			//	adjust = !firstInterval;
			//}
			//if (adjust)
			//{
			//	var deltaRatio = intervalDelayInMilliseconds / (double)intervalInMilliseconds;
			//	if (deltaRatio <= 0.10d)
			//	{
			//		Console.WriteLine($"deltaRatio: deltaRatio {deltaRatio}, {intervalDelayInMilliseconds} / {intervalInMilliseconds}");
			//		intervalDelayInMilliseconds = intervalInMilliseconds;
			//	}
			//}
